...
def extract(policy, r):
    if   "ec2"      in policy or "pet"      in policy: return r.get("InstanceId",        "?"), tag(r)
    elif "volume"   in policy:                          return r.get("VolumeId",           "?"), tag(r)
    elif "snapshot" in policy:                          return r.get("SnapshotId",         "?"), r.get("Description", "")[:60]
    elif "ami"      in policy:                          return r.get("ImageId",            "?"), r.get("Name", "")
    elif "eip"      in policy:                          return r.get("AllocationId",       "?"), r.get("PublicIp", "")
    elif "eni"      in policy:                          return r.get("NetworkInterfaceId", "?"), r.get("Description", "")
    elif "nat"      in policy:                          return r.get("NatGatewayId",       "?"), tag(r)
    elif "security" in policy:                          return r.get("GroupId",            "?"), r.get("GroupName", "")
    elif "key"      in policy:                          return r.get("KeyPairId",          "?"), r.get("KeyName", "")
    elif "log"      in policy:                          return r.get("logGroupName",       "?"), ""
    else:                                               return "?", ""
...
def main():
    ...
    ap.add_argument("--prefix",    default="dryrun-output",
